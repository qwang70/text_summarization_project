...
# Fit
history=model.fit([x_tr,y_tr[:,:-1]], y_tr.reshape(y_tr.shape[0],y_tr.shape[1], 1)[:,1:] ,epochs=args.epoch, callbacks=[es],batch_size=128, validation_data=([x_val,y_val[:,:-1]], y_val.reshape(y_val.shape[0],y_val.shape[1], 1)[:,1:]))
...
with open(SNAPSHOT_NAME + "/result.txt", "w") as f:
    for i in range(0, len(x_test)):
        ...
        f.write("\n")
        f.write("Predicted summary:" + predicted_summary)
        f.write("\n\n")
        cal_metrics(metrics, original_summary, predicted_summary)
